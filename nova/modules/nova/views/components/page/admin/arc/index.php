					...
					<div class="btn-toolbar pull-right">
						<div class="btn-group">
							<?php if (count($r->getComments()) > 0): ?>
								<a href="<?php echo Uri::create('admin/application/review/'.$r->id);?>"  class="btn btn-small tooltip-top" title="<?php echo ucfirst(lang('comments'));?>"><span class="icn icn-50" data-icon="c"></span> <?php echo count($r->getComments());?></a>
							<?php endif;?>

							<?php if (count($r->getVotes('yes')) > 0): ?>
								<a href="<?php echo Uri::create('admin/application/review/'.$r->id);?>"  class="btn btn-small btn-success tooltip-top" title="<?php echo ucfirst(langConcat('yes votes'));?>"><span class="icn" data-icon="."></span> <?php echo count($r->getVotes('yes'));?></a>
							<?php endif;?>

							<?php if (count($r->getVotes('no')) > 0): ?>
								<a href="<?php echo Uri::create('admin/application/review/'.$r->id);?>"  class="btn btn-small btn-danger tooltip-top" title="<?php echo ucfirst(langConcat('no votes'));?>"><span class="icn" data-icon="/"></span> <?php echo count($r->getVotes('no'));?></a>
							<?php endif;?>
						</div>
