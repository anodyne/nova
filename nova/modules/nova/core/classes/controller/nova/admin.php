...
class Controller_Nova_Admin extends Controller_Nova_Base {
	...
	public function action_index()
	{
		if (isset($_POST['submit']))
		{
			...
			if ($validate->check())
			{
				...
				if ($change->saved())
				{
					...
					$this->template->structure->flash = View::factory('admin/pages/flash');
					$this->template->structure->flash->status = 'success';
					$this->template->structure->flash->message = __('Password was successfully changed.');
				...
				else
				{
					...
					$this->template->structure->flash->message = __('Password was not successfully changed. Please try again.');
				}
			}
			else
			{
		...
		$this->template->structure->content = View::factory(Location::view('admin_index', $this->skin, 'admin', 'pages'));
