...
class Fusion_Query {
	...
	public function select()
	{
		...
		if (empty($fields))
		{
			if (empty($this->select))
			{
				...
				if (empty($fields))
				{
					throw new Kohana_Exception('No properties found in model.');
				}
	...
	public function _where($condition, $type = 'and_where')
	{
		...
		{
			throw new Kohana_Exception('Invalid param count for where condition.');
		}
	...
	public function related($relation, $conditions = array())
	{
		...
		if (strpos($relation, '.'))
		{
			...
			foreach ($rels as $r)
			{
				...
				if (empty($rel))
				{
					throw new Kohana_Exception('Relation "'.$r.'" was not found in the model "'.$model.'".');
				}
				$model = $rel->model_to;
			}
		}
		else
		{
			...
			if (empty($rel))
			{
				throw new Kohana_Exception('Relation "'.$relation.'" was not found in the model.');
			}
		}
	...
	public function build_query($query, $columns = array(), $type = 'select')
	{
		...
		foreach ($this->relations as $name => $rel)
		{
			...
			if ($pos = strrpos($name, '.'))
			{
				...
				{
					throw new Kohana_Exception('Trying to get the relation of an unloaded relation, make sure you load the parent relation before any of its children.');
				}
	...
	public function max($column)
	{
		...
		// Get the columns
		$columns = DB::expr('MAX('.Database::instance()->table_prefix().$this->alias.'.'.$column.') AS max_result');
	...
	public function min($column)
	{
		...
		// Get the columns
		$columns = DB::expr('MIN('.Database::instance()->table_prefix().$this->alias.'.'.$column.') AS min_result');
