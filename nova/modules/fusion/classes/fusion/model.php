...
class Fusion_Model implements ArrayAccess, Iterator {
	...
	public static function properties()
	{
		...
		if (empty($properties))
		{
			try
			{
				$properties = Database::instance()->list_columns(static::table());
			}
			catch (Exception $e)
			{
				throw new Kohana_Exception('Listing columns not failed, you have to set the model properties with a '.
	...
	public function _relate($rels = false)
	{
		if ($this->_frozen)
		{
			throw new Kohana_Exception('No changes allowed.');
		}
		...
		{
			throw new Kohana_Exception('Invalid input for _relate(), should be an array.');
		}
	}
	...
	public function & __get($property)
	{
		...
		{
			throw new Kohana_Exception('Property "'.$property.'" not found for '.get_called_class().'.');
		}
	}
		...
		foreach ($this->observers() as $observer => $events)
		{
			if (empty($events) or in_array($event, $events))
			{
				if ( ! class_exists($observer))
				{
					...
					{
						throw new Kohana_Exception($observer);
					}
	...
	{
		Fusion_Observer_Validation::set_fields(get_called_class(), $form);
