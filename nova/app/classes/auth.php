...
class Auth {
	 ...
	 */
	protected static $_allowed_login_attempts = 5;
	
	/**
	 * @var	int		number of seconds the lockout lasts
	 */
	protected static $_lockout_time = 1800;
	...
	public static function init()
	{
		...
		// grab the login attempts limit from the database
		self::$_allowed_login_attempts = (int) Model_Settings::get_settings('login_attempts');
		
		// grab the login lockout time period (in minutes) from the database
		self::$_lockout_time = (int) Model_Settings::get_settings('login_lockout_time') * 60;
	}
	...
	protected static function _check_login_attempts($email)
	{
		...
		if (count($attempts) < self::$_allowed_login_attempts)
		{
			return true;
		}
		else
		{
			...
			if ($timeframe > self::$_lockout_time)
			{
	...
	protected static function _set_session($user)
	{
		...
		self::$_session->set('access', ($user->get_status() == 'inactive') 
			? self::_set_access(Model_AccessRole::INACTIVE) 
			: self::_set_access($user->role_id));
