...
class Install_base extends Controller {
	...
	function modinstall()
	{
		/*
			...
			1 - select which file to use
			2 - include the selected file
			3 - read the data and edit the database
			4 - add a row to nova_system_mods
		*/
		...
		switch ($step)
		{
			case 1:
				...
				/* map the install/mods directory */
				$dir = directory_map(APPFOLDER .'/assets/install/mods/');
				
				foreach ($dir as $key => $value)
				{ /* make sure the index.html and uninstall files are removed from this list */
					if ($value == 'index.html' || substr($value, -14) == '_uninstall.php')
					{
						unset($dir[$key]);
					}
				}
	...
	{
		$data['label'] = array(
			'content_bios' => lang('install_tour_bios_content'),
			'content_genres' => lang('install_tour_genres_content'),
			'content_players' => lang('install_tour_players_content'),
			'content_wiki' => lang('install_tour_wiki_content'),
			'header_bios' => lang('install_tour_header_bios'),
			'header_genres' => lang('install_tour_header_genres'),
			'header_players' => lang('install_tour_header_players'),
			'header_wiki' => lang('install_tour_header_wiki'),
			'whatsnew' => lang('install_tour_whatsnew'),
			'whatsnew_intro' => lang('install_tour_whatsnew_content'),
		);
		
		/* figure out where the view file should be coming from */
		$view_loc = view_location('tour', '_base', 'install');
		
		/* set the title */
		$this->template->write('title', lang('install_title_tour'));
		$this->template->write('label', lang('install_title_tour'));
