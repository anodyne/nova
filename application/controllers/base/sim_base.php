...
|	the department is associated with; fixed bug where the deck
|	listing didn't break up decks by specification item
|
*/
...
class Sim_base extends Controller {
	...
	function awards()
	{
		...
		if ($award == 0)
		{
			...
			/* figure out where the view should be coming from */
			$view_loc = view_location('sim_awards_all', $this->skin, 'main');
		}
		else
		{
			/* get the title */
			...
			if ($award_row !== FALSE)
			{
				...
				{ /* build the awardees data array */
					...
					if ($awardees->num_rows() == 1)
					{ /* make sure the proper singular/plural case is used */
						$times = singular(lang('labels_times'));
					}
					else
					{
						$times = lang('labels_times');
					}
				...
				/* figure out where the view should be coming from */
				$view_loc = view_location('sim_awards_one', $this->skin, 'main');
			}
			else
			{
				/* figure out where the view should be coming from */
		...
		$js_loc = js_location('sim_awards_js', $this->skin, 'main');
	...
	function decks()
	{
		...
		$view_loc = view_location('sim_decks', $this->skin, 'main');
		$js_loc = js_location('sim_decks_js', $this->skin, 'main');
	...
	function departments()
	{
		...
		if ($depts_data->num_rows() > 0)
		{
			...
			{ /* populate main dept data */
				...
				if ($main_positions->num_rows() > 0)
				{
					...
					{ /* populate main dept positions data */
				...
				if ($subdepts->num_rows() > 0)
				{
					...
					{ /* populate the sub dept data */
						...
						if ($sub_positions->num_rows() > 0)
						{
							...
							{ /* populate the data for the sub dept positions */
			...
			/* figure out where the views should be coming from */
			$js_loc = js_location('sim_depts_js', $this->skin, 'main');
		...
		$data['label'] = array(
			'edit_dept' => '[ '. ucfirst(lang('actions_edit') .' '. 
				lang('global_departments')) .' ]',
			'edit_pos' => '[ '. ucfirst(lang('actions_edit') .' '. 
				lang('global_positions')) .' ]',
			'showhide' => lang('labels_showhide_positions'),
			'toggle' => ucwords(lang('actions_toggle') .' '. lang('labels_positions')),
		);
		
		/* figure out where the view should be coming from */
		$view_loc = view_location('sim_depts', $this->skin, 'main');
