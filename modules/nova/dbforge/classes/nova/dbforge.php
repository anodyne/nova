...
abstract class Nova_DBForge {
	...
	{	
		...
		$sql.= self::_escape_identifiers(self::$dbconfig['table_prefix'].$table)." (";
		...
		if (strpos($item, '.') !== FALSE)
		{
			...
			*/
			...
			if (self::$dbconfig['table_prefix'] != '')
			{
				...
				if (substr($parts[$i], 0, strlen(self::$dbconfig['table_prefix'])) != self::$dbconfig['table_prefix'])
				{
					$parts[$i] = self::$dbconfig['table_prefix'].$parts[$i];
				}
